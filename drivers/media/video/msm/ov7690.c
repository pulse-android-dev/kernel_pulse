...
int32_t ov7690_video_config(enum sensor_mode_t       mode,
...
int32_t ov7690_snapshot_config(enum sensor_mode_t mode)
{
    int32_t rc = 0;
...
int32_t ov7690_move_focus(enum sensor_move_focus_t direction,
                          int32_t                  num_steps)
{
...
static int ov7690_sensor_init_done(struct msm_camera_sensor_info *data)
{
...
static int ov7690_probe_init_sensor(struct msm_camera_sensor_info *data)
{
	int rc;
...
int ov7690_sensor_open_init(struct msm_camera_sensor_info *data)
{
	int32_t  rc;
...
int32_t ov7690_set_sensor_mode(enum sensor_mode_t       mode,
                               enum sensor_resolution_t res)
{
    int32_t rc = 0;
...
int ov7690_probe(struct i2c_client *         client,
...
static int __exit ov7690_remove(struct i2c_client *client)
{
...
static struct i2c_driver ov7690_driver =
{
    .id_table = ov7690_id,
    .probe    = ov7690_probe,
    .remove   = __exit_p(ov7690_remove),
    .driver   = {
        .name = "ov7690",
    },
};
int32_t ov7690_slave_elude_func
(
    struct msm_camera_sensor_info *master_sensor_info,
    struct msm_camera_sensor_info *slave_sensor_info

)
{
    int32_t rc = 0;
    
    if(ov7690_client == NULL)
    {
        return -1;
    }
    
    CDBG("ov7690_slave_elude_func\n");
    gpio_direction_output(slave_sensor_info->sensor_pwd, 0);
    mdelay(5);
    gpio_direction_output(master_sensor_info->sensor_reset, 1);
    CDBG("set master_sensor_info->sensor_reset:%d\n",master_sensor_info->sensor_reset);
    /* RESET the sensor image part via I2C command */

    rc = ov7690_i2c_write(ov7690_client->addr,
                          OV7690_REG_RESET_REGISTER,
                          0x80);
    mdelay(5);
    gpio_direction_output(slave_sensor_info->sensor_pwd, 1);
    
    mdelay(100);
    gpio_direction_output(master_sensor_info->sensor_pwd, 0);
    CDBG("clear master_sensor_info->sensor_reset:%d\n",master_sensor_info->sensor_reset);

    mdelay(20);
    return rc;
}
static int32_t ov7690_init(void)
{
	int32_t rc = 0;

	rc = i2c_add_driver(&ov7690_driver);
	if (IS_ERR_VALUE(rc))
		goto init_failure;
	return rc;
...
void ov7690_exit(void)
{
    i2c_del_driver(&ov7690_driver);
}

int ov7690_probe_init(void *dev, void *ctrl)
{
	int rc = 0;
    ...
    info->slave_elude_func = ov7690_slave_elude_func;
