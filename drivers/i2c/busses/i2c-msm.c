...
msm_i2c_interrupt(int irq, void *devid)
{
	...
	if (!dev->msg) {
		...
		if (status & I2C_STATUS_RD_BUFFER_FULL) {
			uint8_t flush_data;
			/* Extra bytes generated. clear buffers */
			dev_err(dev->dev,
...
static int
msm_i2c_low_stuck_recovery(struct msm_i2c_dev *dev)
{
	int gpio_clk = 60;
	int gpio_dat = 61;
	...
	int i;
	uint32_t status = readl(dev->base + I2C_STATUS);
	printk("Low stuck recovery\n");
	/* Configure GPIOs for IO control */
	gpio_tlmm_config(GPIO_CFG(gpio_clk, 0, GPIO_OUTPUT, GPIO_NO_PULL, 
				GPIO_16MA), GPIO_ENABLE);
	gpio_tlmm_config(GPIO_CFG(gpio_dat, 0, GPIO_OUTPUT, GPIO_NO_PULL, 
				GPIO_16MA), GPIO_ENABLE);
	if (status & I2C_STATUS_RD_BUFFER_FULL) {
	...
	gpio_tlmm_config(GPIO_CFG(gpio_clk, 1, GPIO_INPUT, GPIO_PULL_UP, 
				GPIO_16MA), GPIO_ENABLE);
	gpio_tlmm_config(GPIO_CFG(gpio_dat, 1, GPIO_INPUT, GPIO_PULL_UP, 
				GPIO_16MA), GPIO_ENABLE);
	udelay(10);
	return i;		   /* return times when i2c bus recovers idlesse*/				

}

/*
 * when i2c transfer failed, the bus may be busy. and it will lead to 
 * I2C's crash.this function tries to recover the bus from busy state.
 */
static int msm_i2c_low_stuck_recover_bus(struct msm_i2c_dev *dev) {
	uint32_t status;
	int i = msm_i2c_low_stuck_recovery(dev);
...
/*
 *when i2c transfer failed, the write buffer status may be full. and it will lead to I2C's crash.
 *this function tries to recover write buffer to empty.
 */
static int msm_i2c_low_stuck_recover_wb(struct msm_i2c_dev *dev) {
	uint32_t status;
	int i = msm_i2c_low_stuck_recovery(dev);
	status = readl(dev->base + I2C_STATUS);
	if(!(status & I2C_STATUS_WR_BUFFER_FULL)) {
		dev_info(dev->dev, "Write buffer cleared after %d clock cycles, "
			 "status %x, intf %x\n",
			 i, status, readl(dev->base + I2C_INTERFACE_SELECT));
		return 0;
	}

	dev_warn(dev->dev, "Write buffer still full, status %x, intf %x\n",
		 status, readl(dev->base + I2C_INTERFACE_SELECT));
	return -ETIMEDOUT;
}
static int
...
msm_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	while (rem) {
		...
		if (ret) {
			dev_err(dev->dev, "BUS Busy:Trying to recover\n");
			ret = msm_i2c_low_stuck_recover_bus(dev); /* recover bus to idle*/

			if (ret) {
				dev_err(dev->dev, "Couldn't recover Bus\n");
				goto out_err;
			}
		}
		...
		if (ret) {
			dev_err(dev->dev, "Write Buffer full:Trying to recover\n");
			ret = msm_i2c_low_stuck_recover_wb(dev);   /* recover WRITE BUFFER to empty */
			if (ret) {
			     dev_err(dev->dev,"Couldn't cleared write buffer\n");
			     goto out_err;
			}
		}
