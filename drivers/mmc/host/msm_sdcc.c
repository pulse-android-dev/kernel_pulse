...
//       mmc_set_polling(get_wlan_mmc_host(), 1);
	...
	if (enable)
	{
		...
		ret = mpp_config_digital_out(mpp_wlan_12v,
			MPP_CFG(MPP_DLOGIC_LVL_MSME, MPP_DLOGIC_OUT_CTRL_HIGH)); /* pull high */
		if (ret) 
		{
		...
		{
//			printk(KERN_INFO "%s: 1.2V voltage level successed\n",__func__);
		}
		mdelay(20);
		...
		ret = mpp_config_digital_out(mpp_wlan_chip_down,
			MPP_CFG(MPP_DLOGIC_LVL_MSMP, MPP_DLOGIC_OUT_CTRL_HIGH));  /* pull up */
		if (ret) 
		{
		...
		{
            //		printk(KERN_INFO "%s: wlan chip down successed to pull up\n",__func__);
		}
		mdelay(40);
	}
	else 
...
//	       mmc_set_polling(get_wlan_mmc_host(), 0);
	}
